...
test("list-mode pages do not attach swipe while text links are the primary navigation", async () => {
  ...
  for (const source of [taskSource, reminderSource, supplySource]) {
    assert.doesNotMatch(source, /useModeSwipe/);
    assert.doesNotMatch(source, /taskModeSwipeArea/);
    assert.doesNotMatch(source, /ref=\{modeSwipeHandlers\.ref\}/);
  }
});
...
test("list-mode text links keep a large touch target", async () => {
