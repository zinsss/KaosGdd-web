...
test("family polish keeps the baseline compact and overflow-safe", async () => {
  ...
  assert.match(polishCss, /\.familyPage::before\s*\{[\s\S]*?position:\s*fixed;[\s\S]*?top:\s*0;[\s\S]*?z-index:\s*45;[\s\S]*?height:\s*var\(--family-safe-top\);[\s\S]*?pointer-events:\s*none;[\s\S]*?background:\s*#fff8fb;/);
