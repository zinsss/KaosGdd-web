...
test("family polish keeps the baseline compact and overflow-safe", async () => {
  ...
  assert.match(polishCss, /\.familyPage\s*\{[\s\S]*?--family-page-max:\s*var\(--app-column-max-width\);[\s\S]*?width:\s*100%;[\s\S]*?max-width:\s*none;[\s\S]*?min-height:\s*100dvh;[\s\S]*?background:\s*#fff8fb;[\s\S]*?font-size:\s*14px;/);
  assert.match(polishCss, /\.familyCard\s*\{[\s\S]*?width:\s*min\(calc\(100% - \(var\(--app-column-edge-padding\) \* 2\)\), var\(--family-page-max\)\);[\s\S]*?max-width:\s*var\(--family-page-max\);[\s\S]*?margin:\s*0 auto;[\s\S]*?border:\s*0;[\s\S]*?background:\s*transparent;[\s\S]*?box-shadow:\s*none;/);
