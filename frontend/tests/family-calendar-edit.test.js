...
test("family calendar all-day marker defaults the form and renders a top all-day row", async () => {
  ...
  assert.match(calendarCss, /\.familyCalendarFormGrid > label\s*\{[\s\S]*?width:\s*auto;[\s\S]*?max-width:\s*100%;[\s\S]*?min-width:\s*0;[\s\S]*?box-sizing:\s*border-box;/);
  ...
  assert.match(calendarCss, /\.familyCalendarFormTimeSeparator\s*\{[\s\S]*?justify-content:\s*center;[\s\S]*?min-height:\s*42px;/);
