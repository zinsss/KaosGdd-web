...
test("family calendar header keeps a single ordered row with the edit toggle", async () => {
  ...
  const titleIndex = calendarSource.indexOf("<h2>달력</h2>");
  const monthIndex = calendarSource.indexOf('className="familyCalendarMonthControls"');
  const addIndex = calendarSource.indexOf("+ 일정");
  const toggleIndex = calendarSource.indexOf("편집 모드");
  assert.ok(titleIndex !== -1 && monthIndex !== -1 && addIndex !== -1 && toggleIndex !== -1);
  assert.ok(titleIndex < monthIndex, "title should render before month controls");
  assert.ok(monthIndex < addIndex, "month controls should render before + 일정");
  assert.ok(addIndex < toggleIndex, "+ 일정 should render before 편집 모드");

  assert.match(calendarCss, /\.familyCalendarIntro\s*\{[\s\S]*?justify-content:\s*space-between;[\s\S]*?flex-wrap:\s*nowrap;/);
  assert.match(calendarCss, /\.familyCalendarActions\s*\{[\s\S]*?margin-left:\s*auto;[\s\S]*?justify-content:\s*flex-end;[\s\S]*?flex-wrap:\s*nowrap;/);
  assert.match(calendarCss, /\.familyCalendarMonthControls\s*\{[\s\S]*?display:\s*inline-flex;[\s\S]*?align-items:\s*center;[\s\S]*?gap:\s*6px;/);
  assert.match(calendarCss, /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*?\.familyCalendarActions\s*\{[\s\S]*?display:\s*flex;[\s\S]*?justify-content:\s*flex-end;[\s\S]*?flex-wrap:\s*nowrap;/);
  assert.match(calendarCss, /@media\s*\(max-width:\s*420px\)\s*\{[\s\S]*?\.familyCalendarActions\s*\{[\s\S]*?gap:\s*4px;/);
  assert.doesNotMatch(calendarCss, /grid-template-areas:/);
