...
test("family calendar all-day marker defaults the form and renders a top all-day row", async () => {
  ...
  assert.ok(eventFormSource.includes('className="familyCalendarPickerRow familyCalendarFormDateField"'));
  ...
  assert.ok(eventFormSource.includes("familyCalendarFormTimeRow"));
  ...
  assert.match(calendarCss, /\.familyCalendarFormGrid > label,\s*\n\.familyCalendarFormGrid > \.familyCalendarPickerRow,\s*\n\.familyCalendarFormGrid > \.familyCalendarFormTimeRow\s*\{[\s\S]*?width:\s*auto;[\s\S]*?max-width:\s*100%;[\s\S]*?min-width:\s*0;[\s\S]*?box-sizing:\s*border-box;/);
  ...
  assert.match(calendarCss, /\.familyCalendarPickerButton\s*\{[\s\S]*?border-radius:\s*999px;[\s\S]*?min-height:\s*28px;/);
  assert.match(calendarCss, /\.familyCalendarNativePickerInput\s*\{[\s\S]*?position:\s*absolute !important;[\s\S]*?left:\s*-9999px !important;[\s\S]*?width:\s*1px !important;[\s\S]*?opacity:\s*0;[\s\S]*?pointer-events:\s*none;/);
  ...
  assert.match(calendarCss, /\.familyCalendarFormTimeSeparator\s*\{[\s\S]*?justify-content:\s*center;[\s\S]*?min-height:\s*32px;/);
  ...
  assert.match(calendarCss, /@media \(max-width: 640px\)\s*\{[\s\S]*?\.familyCalendarFormGrid\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\);[\s\S]*?row-gap:\s*10px;[\s\S]*?\}[\s\S]*?\.familyCalendarFormDateField,\s*\n\s*\.familyCalendarFormGridAllDay \.familyCalendarFormDateField\s*\{[\s\S]*?grid-column:\s*1 \/ -1;/);
