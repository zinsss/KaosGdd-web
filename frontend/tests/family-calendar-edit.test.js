...
test("family calendar all-day marker defaults the form and renders a top all-day row", async () => {
  ...
  assert.ok(eventFormSource.includes('return `${String(year).slice(-2)}.${String(month).padStart(2, "0")}.${String(day).padStart(2, "0")}(${weekday})`;'));
  ...
  assert.match(calendarCss, /\.familyCalendarDateTimeRow\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex-wrap:\s*wrap;[\s\S]*?align-items:\s*center;[\s\S]*?gap:\s*6px 8px;[\s\S]*?max-width:\s*100%;[\s\S]*?min-width:\s*0;/);
  ...
  assert.match(calendarCss, /\.familyCalendarFormTimeSeparator\s*\{[\s\S]*?justify-content:\s*center;[\s\S]*?min-height:\s*28px;/);
