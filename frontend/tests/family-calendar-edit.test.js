...
test("family calendar all-day marker defaults the form and renders a top all-day row", async () => {
  ...
  assert.ok(eventFormSource.includes("aria-label=\"날짜 선택\""));
  assert.equal((eventFormSource.match(/aria-label="[^"]*시간 선택"/g) || []).length, 2);
  assert.equal((eventFormSource.match(/className="familyCalendarNativePickerInput"/g) || []).length, 3);
  assert.ok(eventFormSource.includes('type="date"'));
  assert.equal((eventFormSource.match(/type="time"/g) || []).length, 2);
