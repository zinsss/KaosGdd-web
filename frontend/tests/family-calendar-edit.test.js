...
${compactCss}`;
  ...
  assert.match(compactCss, /\.familyCalendarWeekSelected\s*\{[\s\S]*?border:\s*1px solid rgba\(214, 128, 157, 0\.24\);[\s\S]*?border-radius:\s*8px;[\s\S]*?padding:\s*8px 6px 6px;[\s\S]*?background:\s*#fffafd;/);
  assert.match(compactCss, /\.familyCalendarExpandedWeek\s*\{[\s\S]*?border:\s*0;[\s\S]*?border-radius:\s*0;[\s\S]*?padding:\s*6px 0 0;[\s\S]*?background:\s*transparent;/);
