...
test("family calendar all-day marker defaults the form and renders a top all-day row", async () => {
  ...
  assert.equal((eventFormSource.match(/className="familyCalendarDateTimeCluster"/g) || []).length, 3);
  assert.equal((eventFormSource.match(/<label className="familyCalendarPickerButton">/g) || []).length, 3);
