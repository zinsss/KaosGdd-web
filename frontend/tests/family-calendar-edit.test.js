...
${compactCss}`;
  ...
  assert.ok(compactCss.includes("grid-template-columns: var(--family-calendar-expanded-rail-width, 14px) repeat(7, minmax(0, 1fr));"));
  ...
  assert.match(compactCss, /\.familyCalendarTimeLabel\s*\{[\s\S]*?color:\s*rgba\(150, 130, 190, 0\.55\);[\s\S]*?font-size:\s*0\.72rem;[\s\S]*?font-weight:\s*600;/);
  ...
  assert.match(compactCss, /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*?\.familyCalendarTimeLabel\s*\{[\s\S]*?color:\s*rgba\(150, 130, 190, 0\.55\);[\s\S]*?font-size:\s*0\.66rem;/);
