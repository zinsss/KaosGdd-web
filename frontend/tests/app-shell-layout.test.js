...
test("Family surface uses independent shell without main navigation", async () => {
  ...
  assert.match(shellCss, /\.appShellMainFamily\s*\{[\s\S]*?height:\s*100dvh;[\s\S]*?min-height:\s*0;[\s\S]*?background:\s*#fff8fb;[\s\S]*?overflow-y:\s*auto;[\s\S]*?-webkit-overflow-scrolling:\s*touch;/);
