...
test("family Roun timetable uses template library plus date-based assignments", async () => {
  ...
  assert.ok(roniSource.includes("resolveFamilyRounPlanForDate(todayDateKey(), rounState)?.id || \"\""), "current applied template should be derived from assignment resolution");
  assert.ok(roniSource.includes("const appliedPlanId = useMemo("), "current applied template should not be stored as a manual active flag");
  ...
  assert.ok(roniSource.includes("appliedPlanId === plan.id ? <span className=\"familyRoniAppliedStar\""), "currently applied row should show a text star");
  assert.ok(roniSource.includes("aria-label=\"현재 적용 중\""), "current applied star should be accessible");
