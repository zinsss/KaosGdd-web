...
test("family tasks use finalized standard Korean labels", async () => {
  ...
  assert.ok(dashboardSource.includes("className=\"familyTaskTitleToggle\""), "task title should be the compact row toggle");
  assert.ok(dashboardSource.includes("className=\"familyTaskChecklistLine\""), "task meta should render as a checkable second line");
  assert.ok(dashboardSource.includes("className=\"familyTaskInlineCheck\""), "task second line should include a completion check control");
  ...
  assert.match(taskCss, /\.familyTaskCardBody\s*\{[\s\S]*?display:\s*grid;[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;/);
  assert.match(taskCss, /\.familyTaskCard h3\s*\{[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;[\s\S]*?text-overflow:\s*ellipsis;/);
  assert.match(taskCss, /\.familyTaskChecklistLine\s*\{[\s\S]*?display:\s*flex;[\s\S]*?align-items:\s*center;[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;/);
  assert.match(taskCss, /\.familyTaskInlineCheck\s*\{[\s\S]*?width:\s*18px;[\s\S]*?height:\s*18px;[\s\S]*?box-shadow:\s*inset 0 0 0 1\.5px rgba\(216, 111, 152, 0\.42\);/);
