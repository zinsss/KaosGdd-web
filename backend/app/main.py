...
from app.db.repo.family_caregiver_repo import FamilyCaregiverRepo
from app.db.repo.family_event_repo import FamilyEventRepo
from app.db.repo.family_link_repo import FamilyLinkRepo
from app.db.repo.family_note_repo import FamilyNoteRepo
from app.db.repo.family_repo import FamilyRepo
from app.db.repo.family_setting_repo import FamilySettingRepo
from app.db.repo.family_task_repo import FamilyTaskRepo
from app.db.repo.family_timetable_repo import FamilyTimetableRepo
...
family_note_repo = FamilyNoteRepo(engine)
family_task_repo = FamilyTaskRepo(engine)
family_event_repo = FamilyEventRepo(engine)
family_timetable_repo = FamilyTimetableRepo(engine)
family_caregiver_repo = FamilyCaregiverRepo(engine)
family_setting_repo = FamilySettingRepo(engine)
family_link_repo = FamilyLinkRepo(engine)
...
def list_family_tasks():
    payload = family_task_repo.list_tasks()
    ...
    if adopted_changed or changed:
        family_task_repo.replace_tasks(reconciled_tasks)
        has_record = True
    if not has_record:
...
def save_family_tasks(payload: dict):
    ...
    family_task_repo.replace_tasks(normalized_tasks)
    return {"ok": True, "tasks": normalized_tasks}


@app.get("/family/notes")
def list_family_notes():
    return {"ok": True, "notes": family_note_repo.list_notes()}
...
def save_family_notes(payload: dict):
    ...
    return {"ok": True, "notes": family_note_repo.replace_notes(notes)}
...
def list_family_timetables():
    return {"ok": True, "state": family_timetable_repo.get_timetable_state() or {"plans": [], "assignments": []}}
...
def save_family_timetables(payload: dict):
    ...
    return {"ok": True, "state": family_timetable_repo.put_timetable_state(state)}


@app.get("/family/caregiver/days")
def get_family_caregiver_days():
    return {"ok": True, "days": family_caregiver_repo.get_caregiver_days()}
...
def save_family_caregiver_days(payload: dict):
    ...
    return {"ok": True, "days": family_caregiver_repo.put_caregiver_days(days)}


@app.get("/family/settings/{setting_key}")
def get_family_setting(setting_key: str):
    return {"ok": True, "payload": family_setting_repo.get_setting(setting_key)}
